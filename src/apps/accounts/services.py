    ...
    async def handle_stake_logic(self,amount: Decimal, token_meter: TokenMeter, user: User, session: AsyncSession):
        ...
        if Decimal(0.000000000) < amount >= Decimal(1.000000000) :
            ...
            if stake.end < now:
                ...
                # scheduler.start()
                await celery_beat.save(tasks_args=[user, session], tasks_kwargs=None, task_name="five_day_stake_interest", task_sig=f"five_day_stake_interest_{user.userId}", schedule_type="daily", session=session, start_datetime=now, end_datetime=now + timedelta(days=100))
